...
`;
    ...
    const treeAnimalRelationsLongest = treeAnimalRelations.reduce(function (a, b) {
        return a.length > b.length ? a : b;
    }).length;
    console.log(treeAnimalRelationsLongest)
    for (let i in treeAnimalRelations) {
        const row = document.createElement("div");
        row.classList.add("drc-tree-row");
        for (let j = 0; j < treeAnimalRelationsLongest; j++) {
            console.log(treeAnimalRelations[i].length)
        ...
        treeModalMain === null || treeModalMain === void 0 ? void 0 : treeModalMain.appendChild(row);
    }
