...
const savedSettings: SettingsTemplate = store.get("settings") ?? {};
...
const createWindow = () => {
    ...
    loadingWindow.on("close", () => {
        if (!finishedLoad) mainWindow.close();
    });
    ...
    */
    ...
    mainWindow.webContents.on("console-message", (event: Event, level: Number, message: string, line: Number, sourceId: string) => {
    ...
    ipcMain.on("captureScreenshot", () => {
        ...
        mainWindow.webContents.capturePage().then((image: any) => {
            try {
...
app.on('window-all-closed', () => {
    ...
    if (fs.existsSync(app.getPath('downloads') + "\\drcupdater.exe")) {
        ...
        fs.unlink(app.getPath('downloads') + "/drcupdater.pkg", (err: Error) => {
            if (err) {
                log.info("An error occurred while deleting the installer. Please manually delete `drcupdater.exe` from your Downloads folder");
                console.error(err);
                return;
